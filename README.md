# Prueba técnica – Análisis de datos

Este repositorio contiene el desarrollo de una prueba técnica de análisis de datos,
organizada en dos tareas independientes:

- **Tarea 1:** Análisis del Registro Nacional de Personas Desaparecidas y No Localizadas (RNPDNO)
- **Tarea 2:** Análisis de datos de homicidios en México

Cada tarea cuenta con su propia carpeta, código, datos y documentación.
